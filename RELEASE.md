...
- **AMD "avanzado" (opcional, sin implementar): [`docs/amd-advanced-pawnio.md`](docs/amd-advanced-pawnio.md)** — Tctl exacto (+~3° sobre el die-average) + "Frequency Limit Global" del PM_Table del SMU, vía PawnIO (misma infra que Intel). Bajo ROI / alto mantenimiento (el PM_Table es RE por familia de CPU); solo si el mercado lo pide. El SDK de Ryzen Master (ya en producción) es el default correcto.
